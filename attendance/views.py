...
def home(request):
    ...
    context['frontpage']=VideoWidget.objects.first()
...
def save_student(request):
    ...
    if form.is_valid():
        ...
        if profile is None:
            UserProfile(user = newUser, dob= data['dob'], contact= data['contact'],user_type=1,gender=data['gender'],department=course.department).save()
        else:
            UserProfile.objects.filter(user = newUser).update(dob= data['dob'], contact= data['contact'],user_type=1,gender=data['gender'],department=course.department)
        print(profile)

        testt = SaveStudent(request.POST)
        if testt.is_valid():
            ss=Student(user=newUser,
